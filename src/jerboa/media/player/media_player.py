...
from PyQt6.QtWidgets import (QApplication, QDialog, QFileDialog, QMainWindow, QSlider, QStyle,
                             QToolBar)
...
class MainWindow(QMainWindow):
  ...
  def __init__(self):
    ...
    open_action = QAction(icon, "&Open...", self, shortcut=QKeySequence.StandardKey.Open, triggered=self.open)
    ...
    style = self.style()
    icon = QIcon.fromTheme("media-playback-start.png", style.standardIcon(QStyle.StandardPixmap.SP_MediaPlay))
    ...
    icon = QIcon.fromTheme("media-playback-pause.png", style.standardIcon(QStyle.StandardPixmap.SP_MediaPause))
    ...
    icon = QIcon.fromTheme("media-playback-stop.png", style.standardIcon(QStyle.StandardPixmap.SP_MediaStop))
    ...
    about_qt_act = QAction("About &Qt", self, triggered=app.aboutQt)
...
if __name__ == '__main__':
  ...
  def start():
    ...
    pts_arr = [1 / 120 * i for i in range(600, 0, -1)]

    def update():
      nonlocal start_time
      current_time = time()
      t = current_time - start_time
      print(f'{t:.3f} vs {pts_arr.pop():.3f}')
      delay = (pts_arr[-1] - t)
