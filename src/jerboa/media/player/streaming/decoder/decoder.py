...
class StreamDemuxer:
  ...
  def pull(self, start_timepoint: float):
    while True:
      self._container.seek(round(max(0, start_timepoint) / self.stream.time_base),
                           stream=self.stream)
  ...
  def _get_stream_for_demuxing(self, stream_idx: int) -> None:
    try:
      ...
      raise IndexError(
          ...
          f'"{self._container.name}" has {len(self._container.streams)} streams') from exc
...
class PipelineNode(ABC, Generic[ParentBaseT]):
  ...
  def __init__(self, parent: ParentBaseT, **_) -> None:
    ...
    if not isinstance(parent, compatible_parent_base):
      raise TypeError(f'Parent type "{type(parent)}" is incompatible! '
...
class DecodingPipeline:
  ...
  def decode(self, start_timepoint: float) -> Iterable[JerboaFrame]:
    self.stream.container.seek(round(max(0, start_timepoint) / self.stream.time_base),
                               stream=self.stream)
  ...
  def pull(self) -> Iterable[TimedFrame]:
    ...
    for next_frame in self.parent.pull():
      if current_frame is not None:
        current_frame = TimedFrame(current_frame,
                                   beg_timepoint=current_frame.time,
                                   end_timepoint=next_frame.time)
      yield current_frame
