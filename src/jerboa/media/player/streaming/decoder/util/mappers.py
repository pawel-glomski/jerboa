...
class AudioMapper:
  ...
  def __init__(self, dst_audio_config: AudioConfig) -> None:
    ...
    self._stretcher = RubberBandStretcher(self._std_audio_config.sample_rate,
  ...
  def map(self, frame: av.AudioFrame, mapping_scheme: RangeMappingResult) -> MappedFrame:
    ...
    if flush:
      if self._last_frame_end_timepoint is not None:
        flushing_packet = std_audio.create_audio_array(self._std_audio_config.channels_num,
      ...
      for audio_segment, modifier in self._cut_according_to_mapping_scheme(frame, mapping_scheme):
  ...
  def _cut_according_to_mapping_scheme(self, frame: av.AudioFrame,
                                       mapping_scheme: RangeMappingResult):
