...
class AlgorithmOption(Generic[T]):
  ...
  def __repr__(self) -> str:
    attributes = (
        attr for attr in dir(self) if not attr.startswith('_') and not attr == 'setter_transformer')
    attributes = (f'{attr}={getattr(self, attr)}' for attr in attributes)
    attributes = ', '.join(attributes)
    return f"AlgorithmOption({attributes})"


def option_number(type_: Type[int] | Type[float],
                  min_value: int | float,
                  max_value: int | float,
                  default_value: int | float,
                  description: str = '') -> AlgorithmOption:
...
def option_float(min_value: float,
                 max_value: float,
                 default_value: float,
                 description: str = '') -> AlgorithmOption:
...
def option_path(  # pylint: disable=W0102:dangerous-default-value
  ...
  def path_transformer(path: Path):
    ...
    if path.suffix not in extensions:
      raise ValueError(f'{what} "{path}" has unexpected extension "{path.suffix}", while should '
